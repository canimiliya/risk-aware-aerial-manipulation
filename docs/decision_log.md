...
| D0038 | 2026-08-05 | ACTIVE | S4-R0-R1 修正完成了解析 COM 反作用、USD MassAPI 质量来源、质量记账和动力学重跑；视觉副本时间采样在当前 Isaac native runtime 原生退出 | readiness=`BLOCKED_S4_R0_VISUAL_EVIDENCE_INCOMPLETE`；不伪造新 PNG/视频/曲线，不关闭 S4，不推进 S5 | 取得稳定的 S3 自定义 STL 视觉回放运行时 |
| D0039 | 2026-08-05 | ACTIVE | S4-R0-R2 独立 P0-P3 矩阵将失败缩小到真实 USD viewport capture/link 默认值更新组合；flatten cache 无标准 Mesh 且仍含 Physics schemas；base/head 完整 pytest 无 head-only failure | readiness=`BLOCKED_S4_R0_R2_NATIVE_VISUAL_PIPELINE_UNRESOLVED`；保留旧 R0 视觉为 legacy，不伪造 R2 12 PNG/3 视频/3 曲线，不关闭 S4，不推进 S5 | 稳定的真实 S3 自定义 STL live viewport 管线或负责人明确的新技术路线授权 |
| D0040 | 2026-08-08 | ACTIVE | S4-R3 只读审计真实 `aerial_manipulator_v2.usd` 及 6 个 composed layers：10 个正质量 arm links、3 个可动 revolute、6 个零限位 revolute，但 0 个闭环 joint；拓扑为 end-effector 星形树，且无 floating UAV base | readiness=`BLOCKED_S4_R3_DELTA_TOPOLOGY_UNRESOLVED`；不创建替代模型，不运行 native runtime，不进入 R4 rotor；legacy surrogate 继续隔离保留 | 项目负责人提供/批准具有浮动 base 与完整 Delta closure joints 的权威物理 USD 资产 |
