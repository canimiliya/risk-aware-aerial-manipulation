...
variant="${1:?usage: s2_r4_run_am_planner.sh smoke_free|loose|nominal|narrow run-id timeout-s}"
run_id="${2:?run id required}"
timeout_s="${3:?timeout seconds required}"
case "$variant" in smoke_free|loose|nominal|narrow) ;; *) exit 64 ;; esac

# Reuse the validated S2-R2 workspace read-only; only S2-R4 run evidence is new.
...
run_dir="$repo/docs/evidence/S2-R4/runtime/$run_id"
...
launch_file="$repo/planner_bridge/scenes/s2_r4_official.launch"
...
{
  printf 'stage=S2-R4\nvariant=%s\nrun_id=%s\ntimeout_s=%s\nworkspace=%s\n' "$variant" "$run_id" "$timeout_s" "$workspace"
  env -u PYTHONPATH -u PYTHONHOME "$conda_root/bin/python" -c 'import sys,torch; print(sys.version); print(torch.__version__); print(torch.cuda.is_available()); print(torch.cuda.get_device_name(0)); print(torch.cuda.get_device_capability(0))'
...
env PYTHONPATH="$PYTHONPATH" /opt/ros/noetic/bin/roscore >"$run_dir/roscore.log" 2>&1 &
roscore_pid=$!
env PYTHONPATH="$PYTHONPATH" /usr/bin/python3 "$map_script" --variant "$variant" >"$run_dir/map.log" 2>&1 &
map_pid=$!
...
env PYTHONPATH="$PYTHONPATH" /usr/bin/python3 "$capture_script" --output-dir "$run_dir" --timeout-s "$timeout_s" >"$run_dir/capture.log" 2>&1 &
capture_pid=$!
sleep 2
env PYTHONHOME="$conda_root" PYTHONPATH="$conda_root/lib/python3.9/site-packages:$PYTHONPATH" "$conda_root/bin/python" /opt/ros/noetic/bin/roslaunch --screen "$launch_file" task:="$variant" >"$run_dir/roslaunch.log" 2>&1 &
launch_pid=$!
