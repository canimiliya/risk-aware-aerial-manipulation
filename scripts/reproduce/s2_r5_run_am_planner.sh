...
variant="${1:?usage: s2_r5_run_am_planner.sh nominal run-id timeout-s}"
...
run_dir="$repo/docs/evidence/S2-R5/runtime/$run_id"
...
launch_file="$repo/planner_bridge/scenes/s2_r5_official.launch"
...
printf 'stage=S2-R5\nvariant=%s\nrun_id=%s\ntimeout_s=%s\nworkspace=%s\n' "$variant" "$run_id" "$timeout_s" "$workspace" >"$run_dir/command.txt"
...
sleep 2
env PYTHONHOME="$conda_root" PYTHONPATH="$conda_root/lib/python3.9/site-packages:$PYTHONPATH" "$conda_root/bin/python" /opt/ros/noetic/bin/roslaunch --screen "$launch_file" task:="$variant" >"$run_dir/roslaunch.log" 2>&1 & launch_pid=$!
set +e
...
set -e
printf '{"variant":"%s","run_id":"%s","capture_exit":%s}\n' "$variant" "$run_id" "$capture_status" > "$run_dir/processes.json"
exit "$capture_status"
