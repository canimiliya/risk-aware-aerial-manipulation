...
def build_point_groups(variant: str) -> Mapping[str, tuple[tuple[float, float, float], ...]]:
    """Return immutable obstacle-wise point groups for the nominal S2 map.

    Groups retain points generated by each original obstacle primitive. They may
    overlap where primitives overlap; their set union is the frozen nominal
    point cloud returned by :func:`build_points`.
    """
    if variant not in VARIANTS:
        raise ValueError(f"unknown variant: {variant}")
    if variant != "nominal":
        raise ValueError("point groups are defined only for the nominal variant")

    target_proxy: set[tuple[float, float, float]] = set()
    main_beam: set[tuple[float, float, float]] = set()
    column: set[tuple[float, float, float]] = set()
    adjacent_obstacle: set[tuple[float, float, float]] = set()

    _official_target_proxy(target_proxy)
    _segment(main_beam, (0.0, -1.20, 1.32), (0.0, 1.20, 1.32), 0.05)
    _segment(column, (0.0, 0.0, 0.15), (0.0, 0.0, 1.32), 0.06)
    _segment(adjacent_obstacle, (0.0, 0.62, 0.35), (0.0, 0.62, 1.65), 0.045)

    return MappingProxyType(
        {
            "TargetProxy": tuple(sorted(target_proxy)),
            "MainBeam": tuple(sorted(main_beam)),
            "Column": tuple(sorted(column)),
            "AdjacentObstacle": tuple(sorted(adjacent_obstacle)),
        }
    )
