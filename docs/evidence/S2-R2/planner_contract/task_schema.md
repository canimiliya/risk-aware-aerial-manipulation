# Official AM-Planner task schema
...
- `src/plan/plan_manage/src/plan_manage.cpp:458-459` initializes `init_state_` and `fin_state_` as `3x3` matrices.
- `src/plan/plan_manage/src/plan_manage.cpp:466-473` maps the start/end contract to `x,y,z` values only.
...
There is no task-field contract for Delta joint values `m1_1,m2_1,m3_1` or for a joint-vector trajectory input.
